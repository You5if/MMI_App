...
export class CostCenterComponent {
  ...
  pTableId: number = 0;
  pUserId: number = 1;
  displayedColumns: string[] = ['select', 'leaveName', 'description', 'groupId'];
  dataSource: CostCenterModel[];
  isLastPage = false;
...
@ViewChild(MatPaginator) paginator!: MatPaginator;
  ...
  constructor(
    private allowanceService: CostCenterService,
  ...
  refreshMe() {
    ...
    this._cf.newGetPageData(this.pTableName, this.pageData, this.sort, this.filter).subscribe((result: CostCenterModel[]) => {
  ...
  onDelete=  (data: CostCenterModel) => {
    // this.router.navigate(['/user']);
    // console.log(this.firstName);
    var dataToSend: CostCenterModel = data // Example data to send
