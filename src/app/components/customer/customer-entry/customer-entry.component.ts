...
import { CustomerModel, CustomerToSendModel } from '../customer.model';
...
export class CustomerEntryComponent {
  ...
  constructor(
    private allowanceServcie: CustomerService,
        ...
        if (param['id'] != '0') {
          ...
          this.allowanceServcie.getRecordEntry(Number(param['id'])).subscribe({
            next: (response: CustomerModel) => {
              ...
              this.supplierId = response.customerId
              this.supplierName = response.custName
...
btnClick=  () => {
  ...
  var dataToSend: CustomerToSendModel = {
    "CustomerId": this.supplierId,
    "CustName": this.supplierName,
  ...
  this.allowanceServcie.sendData(dataToSend).pipe(
    ...
    response => {
      console.log('API Response:', response);
      this.router.navigate(['/system/customer'], { relativeTo: this.activeRoute.parent });
