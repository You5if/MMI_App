...
import { JournalChildModel } from '../journal-models/journal-child.model';
...
@Component({
  selector: 'app-journal-entry-entry',
  templateUrl: './journal-entry-entry.component.html',
  styleUrl: './journal-entry-entry.component.css'
})
export class JournalEntryEntryComponent {
  ...
  costCenId: number = 1
          ...
          if (param['id'] != '0') {
            ...
            this.journalService.getRecordEntry(Number(param['id'])).subscribe({
              next: (response) => {
                ...
                this.journalId = res.JournEntryId
                this.journaCode = res.JournCode
                this.journaDate = res.JournDate
                this.costCenId = res.CostCenId
                this.childs = res.journDetEntries
    ...
    deleteTenure(loanChild: JournalChildModel) {
    ...
    addNewChild() {
          ...
          const additionalLoanChild: JournalChildModel = {
            "JournDetId": 0,
      "JournEntryId": this.journalId,
      "AccountId": 103,
      "DrAmt": 0.00,
      "CrAmt": 0.00,
      "Narration": "",
  ...
  btnClick=  () => {
    ...
    var dataToSend: JournalToSendModel = {
      journEntryEntry: {
          "JournEntryId": this.journalId,
        "JournCode": this.journaCode,
        "JournDate": this.journaDate,
    ...
    this.journalService.sendData(dataToSend).pipe(
      ...
      response => {
        ...
        this.router.navigate(['/system/journal'], { relativeTo: this.activeRoute.parent });
