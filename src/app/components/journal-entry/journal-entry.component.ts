...
export class JournalEntryComponent {
  ...
  pTableId: number = 0;
  pUserId: number = 1;
  displayedColumns: string[] = ['select', 'journCode', 'journDate'];
  dataSource: any;
  isLastPage = false;
...
@ViewChild(MatPaginator) paginator!: MatPaginator;
  ...
  constructor(
    private loanService: JournalEntryService,
  ...
  refreshMe() {
    ...
    this._cf.newGetPageData(this.pTableName, this.pageData, this.sort, this.filter).subscribe((result: JournalModel[]) => {
  ...
  onDelete=  (data: JournalModel) => {
    // this.router.navigate(['/user']);
    // console.log(this.firstName);
    var dataToSend: JournalModel = data // Example data to send
