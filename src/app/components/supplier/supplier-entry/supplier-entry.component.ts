...
import { SupplierModel, SupplierToSendModel } from '../supplier.model';
...
@Component({
  ...
  styleUrl: './supplier-entry.component.css'
})
export class SupplierEntryComponent {
  ...
  constructor(
    private allowanceServcie: SupplierService,
        ...
        if (param['id'] != '0') {
          ...
          this.allowanceServcie.getRecordEntry(Number(param['id'])).subscribe({
            next: (response: SupplierModel) => {
              ...
              this.supplierName = response.suppName
...
btnClick=  () => {
  ...
  var dataToSend: SupplierToSendModel = {
  ...
  this.allowanceServcie.sendData(dataToSend).pipe(
    ...
    response => {
      console.log('API Response:', response);
      this.router.navigate(['/system/supplier'], { relativeTo: this.activeRoute.parent });
