...
import { FinalSettleModel } from './final-settlement.model';
...
@Component({
  ...
  styleUrl: './final-settlement.component.css'
})
...
@ViewChild(MatPaginator) paginator!: MatPaginator;
  ...
  refreshMe() {
    ...
    this._cf.newGetPageData(this.pTableName, this.pageData, this.sort, this.filter).subscribe((result: FinalSettleModel[]) => {
  ...
  onDelete=  (data: FinalSettleModel) => {
    ...
    var dataToSend: FinalSettleModel = data // Example data to send
    ...
    if(this.dialog.openDialogs.length==0){
     ...
     dialogRef.afterClosed().subscribe((result: boolean) => {
      console.log(result);
      if (result) {
        this.finalSettleService.deleteRecord(dataToSend).pipe(
          this.toast.observe({
