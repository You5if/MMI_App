...
@ViewChild(MatPaginator) paginator!: MatPaginator;
  ...
  constructor(
    private leaveBalService: LeaveBalanceService,
  ...
  refreshMe() {
    ...
    this._cf.newGetPageData(this.pTableName, this.pageData, this.sort, this.filter).subscribe((result: LeaveBalanceModel[]) => {
  ...
  onDelete=  (data: LeaveBalanceModel) => {
    // this.router.navigate(['/user']);
    // console.log(this.firstName);
    var dataToSend: LeaveBalanceModel = data // Example data to send
    ...
    if(this.dialog.openDialogs.length==0){
     ...
     dialogRef.afterClosed().subscribe((result: boolean) => {
      console.log(result);
      if (result) {
        this.leaveBalService.deleteRecord(dataToSend).pipe(
          this.toast.observe({
