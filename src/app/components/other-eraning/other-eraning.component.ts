...
import { EarnModel } from './other-earning.model';
...
@Component({
  selector: 'app-other-eraning',
  templateUrl: './other-eraning.component.html',
  styleUrl: './other-eraning.component.css'
})
export class OtherEraningComponent {
  ...
  pTableId: number = 0;
  pUserId: number = 1;
  displayedColumns: string[] = ['Employees', 'Month', 'Year', 'Amount', 'Remarks', 'select'];
  dataSource: EarnModel[];
  isLastPage = false;
...
@ViewChild(MatPaginator) paginator!: MatPaginator;
  ...
  constructor(
    private earnService: OtherEraningService,
  ...
  refreshMe() {
    ...
    this._cf.newGetPageData(this.pTableName, this.pageData, this.sort, this.filter).subscribe((result: EarnModel[]) => {
  ...
  onDelete=  (data: EarnModel) => {
    // this.router.navigate(['/user']);
    // console.log(this.firstName);
    var dataToSend: EarnModel = data // Example data to send
    ...
    if(this.dialog.openDialogs.length==0){
     ...
     dialogRef.afterClosed().subscribe((result: boolean) => {
      console.log(result);
      if (result) {
        this.earnService.deleteRecord(dataToSend).pipe(
          this.toast.observe({
