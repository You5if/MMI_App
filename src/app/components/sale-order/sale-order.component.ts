...
export class SaleOrderComponent {
  ...
  pTableId: number = 0;
  pUserId: number = 1;
  displayedColumns: string[] = ['journCode', 'journDate', 'tax', 'select'];
  dataSource: any;
  isLastPage = false;
...
@ViewChild(MatPaginator) paginator!: MatPaginator;
  ...
  refreshMe() {
    ...
    this._cf.newGetPageData(this.pTableName, this.pageData, this.sort, this.filter).subscribe((result: SaleModel[]) => {
